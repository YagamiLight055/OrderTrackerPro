...
const OrdersList: React.FC<Props> = ({ onEdit }) => {
  ...
  // State for the advanced gallery viewer
  ...
  const filteredOrders = useMemo(() => {
    ...
    return allOrders.filter(o => {
      ...
      return matchesSearch && matchesCity && (!filterCustomer || o.customer === filterCustomer) && (!filterMaterial || o.material === filterMaterial) && matchesStatus && (!start || orderDate >= start) && (!end || orderDate <= end);
    });
  ...
  return (
    <div className="space-y-6">
      {/* Search and Filters */}
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 sticky top-[80px] z-20 space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {(searchTerm || filterCity || filterStatus) && (
            <button 
              onClick={() => {setSearchTerm(''); setFilterCity(''); setFilterStatus(''); setFilterCustomer(''); setFilterMaterial('');}} 
              className="px-6 py-4 bg-indigo-50 text-indigo-600 rounded-2xl font-black text-sm whitespace-nowrap"
            >
              Reset
            </button>
          )}
